...
namespace tests::client {
    ...
    TEST(ClientUiTest, EditorWindow_SaveButton_UploadsContentAndShowsSavedVersion) {
        ...
        auto* window = ui::EditorWindow::createEditable(runtime,
                                                        42,
                                                        100500,
                                                        QStringLiteral("/docs/a.txt"),
                                                        QByteArray{"old content"});
        window->show();
        ...
        ASSERT_TRUE(waitUntil([&]() {
            return statusLabel->text().contains(QStringLiteral("Saved version 2"));
        }, 5000));
        QTest::mouseClick(releaseButton, Qt::LeftButton);
        ASSERT_TRUE(waitUntil([&]() {
            return statusLabel->text().contains(QStringLiteral("Lock released"));
        }, 5000));
