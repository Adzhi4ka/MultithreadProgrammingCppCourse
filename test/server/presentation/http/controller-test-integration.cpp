...
void ControllerIntegrationTest::SetUp() {
    ...
    m_userService =
        std::make_unique<domain::services::UserService>(*m_database, *m_userRepo, *m_groupRepo, *m_userGroupRepo);
    m_groupService = std::make_unique<domain::services::GroupService>(*m_database, *m_groupRepo, *m_userGroupRepo);
    m_fileAclService =
        std::make_unique<domain::services::FileAclService>(*m_database, *m_fileAclRepo, *m_userGroupRepo);
    m_fileService = std::make_unique<domain::services::FileService>(*m_database, *m_fileRepo, *m_fileVersionRepo,
                                                                    *m_userRepo, *m_groupRepo, *m_fileAclRepo);
    m_fileVersionService =
        std::make_unique<domain::services::FileVersionService>(*m_database, *m_fileRepo, *m_fileVersionRepo);
    m_fileLockService = std::make_unique<domain::services::FileLockService>(*m_database, *m_fileLockRepo);
    ...
    m_authController =
        std::make_unique<presentation::http::AuthController>(*m_userService, *m_tokenStore, *m_appThreadPool);
    m_groupController =
        std::make_unique<presentation::http::GroupController>(*m_groupService, *m_tokenStore, *m_appThreadPool);
    m_fileAclController =
        std::make_unique<presentation::http::FileAclController>(*m_fileAclService, *m_tokenStore, *m_appThreadPool);
    m_fileController = std::make_unique<presentation::http::FileController>(
        *m_fileService, *m_fileContentService, *m_fileAclService, *m_tokenStore, *m_appThreadPool);
    m_fileVersionController = std::make_unique<presentation::http::FileVersionController>(
        *m_fileVersionService, *m_fileContentService, *m_fileAclService, *m_tokenStore, *m_appThreadPool);
    m_fileLockController =
        std::make_unique<presentation::http::FileLockController>(*m_fileLockService, *m_tokenStore, *m_appThreadPool);
    m_notificationController =
        std::make_unique<presentation::http::NotificationController>(*m_sessionRegistry, *m_tokenStore);
    ...
    m_listener = std::make_shared<infrastructure::http::Listener>(
        *m_ioc, net::ip::tcp::endpoint{net::ip::tcp::v4(), kPort}, *m_router);
    m_listener->run();
...
http::response<http::string_body> ControllerIntegrationTest::request(http::verb method, std::string target,
                                                                     std::string body, std::string bearerToken) {
    net::io_context ioc;
...
AuthSession ControllerIntegrationTest::registerUser(const std::string& login, const std::string& password) {
    json::object body{{"login", login}, {"password", password}};

    auto response = request(http::verb::post, "/api/auth/register", json::serialize(body));

    EXPECT_EQ(response.result(), http::status::created) << response.body();

    auto value = parseJson(response).as_object();

    return AuthSession{.userId = value.at("userId").as_int64(),
...
AuthSession ControllerIntegrationTest::loginUser(const std::string& login, const std::string& password) {
    ...
    auto response = request(http::verb::post, "/api/auth/login", json::serialize(body));
...
int64_t ControllerIntegrationTest::createGroup(const std::string& token, const std::string& name) {
    ...
    auto response = request(http::verb::post, "/api/groups", json::serialize(body), token);
...
void ControllerIntegrationTest::addUserToGroup(const std::string& token, int64_t userId, int64_t groupId) {
    json::object body{{"userId", userId}, {"groupId", groupId}};

    auto response = request(http::verb::post, "/api/groups/members", json::serialize(body), token);

    EXPECT_EQ(response.result(), http::status::ok) << response.body();
}

int64_t ControllerIntegrationTest::createFile(const std::string& token, const std::string& logicalName,
                                              uint32_t maxVersionCount) {
    json::object body{{"logicalName", logicalName}, {"maxVersionCount", maxVersionCount}};

    auto response = request(http::verb::post, "/api/files", json::serialize(body), token);
...
void ControllerIntegrationTest::setGroupAcl(const std::string& token, int64_t fileId, int64_t groupId,
                                            std::string_view aclLevel) {
    json::object body{{"fileId", fileId}, {"groupId", groupId}, {"aclLevel", std::string(aclLevel)}};

    auto response = request(http::verb::put, "/api/file-acl/groups", json::serialize(body), token);

    EXPECT_EQ(response.result(), http::status::ok) << response.body();
}

int64_t ControllerIntegrationTest::createVersion(const std::string& token, int64_t fileId,
                                                 const std::string& logicalNameSnapshot) {
    json::object body{{"fileId", fileId}, {"logicalNameSnapshot", logicalNameSnapshot}};

    auto response = request(http::verb::post, "/api/file-versions", json::serialize(body), token);
