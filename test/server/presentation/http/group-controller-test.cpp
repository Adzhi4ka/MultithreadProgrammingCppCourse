...
TEST_F(ControllerIntegrationTest, GroupController_CreateMembershipAndQueries_Work) {
    ...
    auto getByIdResponse =
        request(http::verb::get, "/api/groups/by-id?groupId=" + std::to_string(groupId), {}, alice.token);
    ...
    auto userGroupsResponse =
        request(http::verb::get, "/api/groups/by-user?userId=" + std::to_string(bob.userId), {}, alice.token);
    ...
    auto groupUsersResponse =
        request(http::verb::get, "/api/groups/users?groupId=" + std::to_string(groupId), {}, alice.token);
