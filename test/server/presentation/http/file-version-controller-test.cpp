...
namespace tests {
    ...
    TEST_F(ControllerIntegrationTest, FileVersionController_WorksWithTwoLargePhysicalFiles) {
        ...
        auto currentV1Response = request(http::verb::get,
                                         "/api/file-versions/current?fileId=" + std::to_string(fileId),
                                         {},
                                         bob.token);
        ...
        auto currentV2Response = request(http::verb::get,
                                         "/api/file-versions/current?fileId=" + std::to_string(fileId),
                                         {},
                                         bob.token);
        ...
        auto allVersionsResponse = request(http::verb::get,
                                           "/api/file-versions?fileId=" + std::to_string(fileId),
                                           {},
                                           bob.token);
    ...
    TEST_F(ControllerIntegrationTest, FileVersionController_RequiresWriteAclForNewVersion) {
        ...
        json::object body{
            {"fileId", fileId},
            {"logicalNameSnapshot", "docs/versioned-v2.txt"}
        };

        auto forbiddenCreateVersionResponse = request(http::verb::post,
                                                      "/api/file-versions",
                                                      json::serialize(body),
                                                      bob.token);
        EXPECT_EQ(forbiddenCreateVersionResponse.result(), http::status::forbidden);

        auto allowedReadResponse = request(http::verb::get,
                                           "/api/file-versions/current?fileId=" + std::to_string(fileId),
                                           {},
                                           bob.token);
