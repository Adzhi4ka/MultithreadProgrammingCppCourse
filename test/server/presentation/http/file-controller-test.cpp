...
namespace tests {
    ...
    TEST_F(ControllerIntegrationTest, FileController_RespectsAclForReadAndWriteOperations) {
        ...
        auto forbiddenGetResponse = request(http::verb::get,
                                            "/api/files/by-id?fileId=" + std::to_string(fileId),
                                            {},
                                            bob.token);
        ...
        auto allowedGetResponse = request(http::verb::get,
                                          "/api/files/by-id?fileId=" + std::to_string(fileId),
                                          {},
                                          bob.token);
        ...
        json::object renameBody{
            {"fileId", fileId},
            {"newLogicalName", "docs/report-renamed.txt"}
        };

        auto forbiddenRenameResponse = request(http::verb::put,
                                               "/api/files/rename",
                                               json::serialize(renameBody),
                                               bob.token);
        EXPECT_EQ(forbiddenRenameResponse.result(), http::status::forbidden);

        setGroupAcl(alice.token, fileId, sharedGroupId, "read_write");

        auto allowedRenameResponse = request(http::verb::put,
                                             "/api/files/rename",
                                             json::serialize(renameBody),
                                             bob.token);
        ...
        auto deleteResponse = request(http::verb::delete_,
                                      "/api/files?fileId=" + std::to_string(fileId),
                                      {},
                                      bob.token);
    ...
    TEST_F(ControllerIntegrationTest, FileAclController_SetGetEffectiveAndList_Work) {
        ...
        auto groupAclResponse = request(http::verb::get,
                                        "/api/file-acl/groups?fileId=" + std::to_string(fileId) +
                                        "&groupId=" + std::to_string(readersGroupId),
                                        {},
                                        alice.token);
        ...
        auto userAclResponse = request(http::verb::get,
                                       "/api/file-acl/users?fileId=" + std::to_string(fileId) +
                                       "&userId=" + std::to_string(bob.userId),
                                       {},
                                       alice.token);
        ...
        auto byFileResponse = request(http::verb::get,
                                      "/api/file-acl/by-file?fileId=" + std::to_string(fileId),
                                      {},
                                      alice.token);
