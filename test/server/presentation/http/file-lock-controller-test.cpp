...
TEST_F(ControllerIntegrationTest, FileLockController_AcquireRenewRelease_Work) {
    ...
    json::object acquireBody{{"fileId", fileId}, {"lockDurationSec", 120}};

    auto acquireResponse = request(http::verb::post, "/api/file-locks", json::serialize(acquireBody), alice.token);
    ...
    auto activeResponse =
        request(http::verb::get, "/api/file-locks/active?fileId=" + std::to_string(fileId), {}, alice.token);
    ...
    json::object renewBody{{"fileId", fileId}, {"lockToken", lockToken}, {"lockDurationSec", 180}};

    auto renewResponse = request(http::verb::put, "/api/file-locks/renew", json::serialize(renewBody), alice.token);
    EXPECT_EQ(renewResponse.result(), http::status::ok);

    auto releaseResponse =
        request(http::verb::delete_,
                "/api/file-locks?fileId=" + std::to_string(fileId) + "&lockToken=" + std::to_string(lockToken), {},
                alice.token);
