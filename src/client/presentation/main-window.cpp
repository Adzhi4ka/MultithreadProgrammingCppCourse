...
MainWindow::MainWindow(application::ClientRuntime& runtime, domain::models::UserSession session, QWidget* parent)
    : QMainWindow(parent), m_runtime(runtime), m_session(std::move(session)) {
    buildUi();
...
void MainWindow::buildUi() {
    ...
    connect(m_fileTree, &FileTreeWidget::selectedFileChanged, this,
            [this]() { m_infoPanel->setFile(m_fileTree->selectedFile()); });

    connect(m_fileTree, &FileTreeWidget::fileActivated, this, [this](qint64) { openSelectedReadOnly(); });
...
void MainWindow::createFile() {
    ...
    groupsLoadedConnection =
        connect(&m_runtime, &application::ClientRuntime::currentUserGroupsLoaded, &dialog,
                [this, &dialog](qint64 currentUserId, ApiResult<std::vector<domain::models::Group>> result) mutable {
...
void MainWindow::renameFile() {
    ...
    bool ok = false;
    const auto newName = QInputDialog::getText(this, QStringLiteral("Rename file"), QStringLiteral("New logical name"),
                                               QLineEdit::Normal, file->fullLogicalName, &ok)
                             .trimmed();
...
void MainWindow::deleteFile() {
    ...
    if (QMessageBox::question(this, QStringLiteral("Delete file"),
...
void MainWindow::handleCurrentDownloaded(qint64 fileId, ApiResult<QByteArray> result) {
    if (m_pendingEditFile && m_pendingEditFile->id == fileId && m_pendingEditLock) {
        ...
        auto* editor =
            EditorWindow::createEditable(m_runtime, file.id, lock.lockToken, file.fullLogicalName, std::move(*result));
...
void MainWindow::handleVersionDownloaded(qint64 versionId, ApiResult<QByteArray> result) {
    ...
    auto* editor =
        EditorWindow::createReadOnly(m_runtime, QStringLiteral("version %1").arg(versionId), std::move(*result));
    registerEditor(editor);
    editor->show();
}
...
std::optional<domain::models::RemoteFile> MainWindow::selectedFile() const { return m_fileTree->selectedFile(); }

void MainWindow::registerEditor(EditorWindow* window) {
    m_editorWindows.push_back(QPointer<EditorWindow>{window});

    connect(window, &QObject::destroyed, this, [this, window]() {
        m_editorWindows.erase(std::remove_if(m_editorWindows.begin(), m_editorWindows.end(),
