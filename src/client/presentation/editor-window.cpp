...
EditorWindow* EditorWindow::createReadOnly(application::ClientRuntime& runtime, QString title, QByteArray content,
                                           QWidget* parent) {
...
EditorWindow* EditorWindow::createEditable(application::ClientRuntime& runtime, qint64 fileId, qint64 lockToken,
                                           QString logicalName, QByteArray content, QWidget* parent) {
    auto* window = new EditorWindow(runtime, std::move(logicalName), std::move(content), fileId, lockToken, parent);
    window->setAttribute(Qt::WA_DeleteOnClose);
    return window;
}

EditorWindow::EditorWindow(application::ClientRuntime& runtime, QString title, QByteArray content,
                           std::optional<qint64> fileId, std::optional<qint64> lockToken, QWidget* parent)
    : QMainWindow(parent), m_runtime(runtime), m_title(std::move(title)), m_fileId(fileId), m_lockToken(lockToken) {
    buildUi(content);
