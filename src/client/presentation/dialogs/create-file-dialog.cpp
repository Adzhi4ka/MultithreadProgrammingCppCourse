...
CreateFileDialog::CreateFileDialog(QString currentUserLogin, QWidget* parent) : QDialog(parent) {
    ...
    auto* hintLabel =
        new QLabel(QStringLiteral("The file will be created by %1. The selected group will receive read/write access.")
...
QString CreateFileDialog::logicalName() const { return m_logicalNameEdit->text().trimmed(); }

quint32 CreateFileDialog::maxVersionCount() const { return static_cast<quint32>(m_maxVersionsSpin->value()); }

qint64 CreateFileDialog::selectedGroupId() const { return m_groupCombo->currentData().toLongLong(); }

QString CreateFileDialog::selectedGroupName() const { return m_groupCombo->currentText(); }
