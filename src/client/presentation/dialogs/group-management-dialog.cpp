...
GroupManagementDialog::GroupManagementDialog(application::ClientRuntime& runtime, domain::models::UserSession session,
                                             QWidget* parent)
    : QDialog(parent), m_runtime(runtime), m_session(std::move(session)) {
    buildUi();
...
void GroupManagementDialog::connectRuntimeSignals() {
    connect(&m_runtime, &application::ClientRuntime::currentUserGroupsLoaded, this,
            &GroupManagementDialog::handleCurrentUserGroupsLoaded);
    connect(&m_runtime, &application::ClientRuntime::groupUsersLoaded, this,
            &GroupManagementDialog::handleGroupUsersLoaded);
    connect(&m_runtime, &application::ClientRuntime::userAddedToGroup, this,
            &GroupManagementDialog::handleUserAddedToGroup);
    connect(&m_runtime, &application::ClientRuntime::userRemovedFromGroup, this,
            &GroupManagementDialog::handleUserRemovedFromGroup);
...
void GroupManagementDialog::removeSelectedUserFromGroup() {
    ...
    if (*userId == m_session.userId) {
        if (QMessageBox::question(this, QStringLiteral("Remove yourself"),
...
void GroupManagementDialog::createGroup() {
    bool ok = false;
    const auto name = QInputDialog::getText(this, QStringLiteral("Create group"), QStringLiteral("Group name"),
...
void GroupManagementDialog::handleGroupUsersLoaded(qint64 groupId,
                                                   ApiResult<std::vector<domain::models::UserProfile>> result) {
