...
using namespace client::domain::models;
using namespace client::domain::services;
using namespace client::infrastructure::repositories;
...
ClientRuntime::ClientRuntime(QUrl baseUrl, QObject* parent)
    : QObject(parent),
      ...
      m_sessionRepository(std::make_unique<SessionRepository>()),
      m_fileRepository(std::make_unique<FileRepository>()),
      m_groupRepository(std::make_unique<GroupRepository>()) {
    ...
    m_authService = std::make_unique<AuthService>(*m_networkWorker, *m_internalContext, *this, *m_sessionRepository);
    m_fileCatalogService =
        std::make_unique<FileCatalogService>(*m_networkWorker, *m_internalContext, *this, *m_fileRepository);
    m_fileWorkspaceService =
        std::make_unique<FileWorkspaceService>(*m_networkWorker, *m_internalContext, *this, *m_fileRepository);
    m_groupSharingService =
        std::make_unique<GroupSharingService>(*m_networkWorker, *m_internalContext, *this, *m_groupRepository);
    m_notificationService = std::make_unique<NotificationService>(*m_networkWorker, *m_internalContext, *this);
...
void ClientRuntime::authenticate(QUrl baseUrl, QString login, QString password, bool registration) {
    postInternal([this, baseUrl = std::move(baseUrl), login = std::move(login), password = std::move(password),
                  registration]() mutable {
        m_authService->authenticate(
            std::move(baseUrl), std::move(login), std::move(password), registration,
            [this](ApiResult<UserSession> result) mutable { emit authenticationFinished(std::move(result)); });
    });
}

void ClientRuntime::setSession(UserSession session) {
    postInternal([this, session = std::move(session)]() mutable { m_authService->setSession(std::move(session)); });
}
...
void ClientRuntime::loadFilesWithMeta(qint64 currentUserId) {
    postInternal([this, currentUserId]() {
        m_fileCatalogService->refreshFiles(currentUserId,
                                           [this, currentUserId](ApiResult<std::vector<RemoteFile>> result) mutable {
...
void ClientRuntime::createFile(QString logicalName, quint32 maxVersionCount, qint64 ownerGroupId) {
    postInternal([this, logicalName = std::move(logicalName), maxVersionCount, ownerGroupId]() mutable {
        m_fileCatalogService->createFile(
            std::move(logicalName), maxVersionCount, ownerGroupId,
            [this](ApiResult<RemoteFile> result) mutable { emit fileCreated(std::move(result)); });
    });
}
...
void ClientRuntime::renameFile(qint64 fileId, QString newLogicalName) {
    postInternal([this, fileId, newLogicalName = std::move(newLogicalName)]() mutable {
        m_fileCatalogService->renameFile(
            fileId, std::move(newLogicalName),
            [this, fileId](ApiResult<RemoteFile> result) mutable { emit fileRenamed(fileId, std::move(result)); });
    });
}

void ClientRuntime::deleteFile(qint64 fileId) {
    postInternal([this, fileId]() {
        m_fileCatalogService->deleteFile(
            fileId, [this, fileId](ApiResult<void> result) mutable { emit fileDeleted(fileId, std::move(result)); });
    });
}

void ClientRuntime::downloadCurrent(qint64 fileId) {
    postInternal([this, fileId]() {
        m_fileWorkspaceService->downloadCurrent(fileId, [this, fileId](ApiResult<QByteArray> result) mutable {
...
void ClientRuntime::uploadCurrent(qint64 fileId, QByteArray content) {
    postInternal([this, fileId, content = std::move(content)]() mutable {
        m_fileWorkspaceService->uploadCurrent(
            fileId, std::move(content),
            [this, fileId](ApiResult<FileVersion> result) mutable { emit currentUploaded(fileId, std::move(result)); });
    });
}

void ClientRuntime::downloadVersion(qint64 versionId) {
    postInternal([this, versionId]() {
        m_fileWorkspaceService->downloadVersion(versionId, [this, versionId](ApiResult<QByteArray> result) mutable {
...
void ClientRuntime::acquireLock(qint64 fileId, qint64 lockDurationSec) {
    postInternal([this, fileId, lockDurationSec]() {
        m_fileWorkspaceService->acquireLock(
            fileId, lockDurationSec,
            [this, fileId](ApiResult<FileLock> result) mutable { emit lockAcquired(fileId, std::move(result)); });
    });
}
...
void ClientRuntime::renewLock(qint64 fileId, qint64 lockToken, qint64 lockDurationSec) {
    postInternal([this, fileId, lockToken, lockDurationSec]() {
        m_fileWorkspaceService->renewLock(fileId, lockToken, lockDurationSec,
...
void ClientRuntime::releaseLock(qint64 fileId, qint64 lockToken) {
    postInternal([this, fileId, lockToken]() {
        m_fileWorkspaceService->releaseLock(fileId, lockToken,
                                            [this, fileId, lockToken](ApiResult<void> result) mutable {
...
void ClientRuntime::loadVersions(qint64 fileId) {
    postInternal([this, fileId]() {
        m_fileWorkspaceService->loadVersions(fileId,
                                             [this, fileId](ApiResult<std::vector<FileVersion>> result) mutable {
...
void ClientRuntime::loadCurrentUserGroups(qint64 currentUserId) {
    postInternal([this, currentUserId]() {
        m_groupSharingService->loadCurrentUserGroups(
            currentUserId, [this, currentUserId](ApiResult<std::vector<Group>> result) mutable {
...
void ClientRuntime::loadGroupUsers(qint64 groupId) {
    postInternal([this, groupId]() {
        m_groupSharingService->loadGroupUsers(groupId,
                                              [this, groupId](ApiResult<std::vector<UserProfile>> result) mutable {
...
void ClientRuntime::addUserToGroup(QString login, qint64 groupId) {
    postInternal([this, login = std::move(login), groupId]() mutable {
        ...
        m_groupSharingService->addUserToGroup(std::move(login), groupId,
...
void ClientRuntime::removeUserFromGroup(qint64 userId, qint64 groupId) {
    postInternal([this, userId, groupId]() {
        m_groupSharingService->removeUserFromGroup(userId, groupId,
...
void ClientRuntime::createGroupForCurrentUser(QString name, qint64 currentUserId) {
    postInternal([this, name = std::move(name), currentUserId]() mutable {
        m_groupSharingService->createGroupForCurrentUser(
            std::move(name), currentUserId,
            [this](ApiResult<Group> result) mutable { emit groupCreated(std::move(result)); });
    });
}
...
void ClientRuntime::startNotifications() {
    postInternal([this]() {
        m_notificationService->start(
            [this](ApiResult<NotificationEvent> result) mutable { emit notificationReceived(std::move(result)); });
    });
}
...
void ClientRuntime::stopNotifications() {
    ...
    postInternal([this]() { m_notificationService->stop(); });
}
