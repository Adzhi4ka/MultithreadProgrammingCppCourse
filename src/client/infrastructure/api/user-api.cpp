...
UserApi::UserApi(ApiClient& apiClient, QObject* parent) : QObject(parent), m_apiClient(apiClient) {}
...
void UserApi::getById(qint64 userId, std::function<void(ApiResult<UserProfile>)> callback) {
    ...
    m_apiClient.get(QStringLiteral("/api/users/by-id"), query,
...
void UserApi::getByLogin(const QString& login, std::function<void(ApiResult<UserProfile>)> callback) {
    ...
    m_apiClient.get(QStringLiteral("/api/users/by-login"), query,
                    [callback = std::move(callback)](RawApiResponse response) mutable {
                        callback(parseUserProfileResponse(response));
                    });
}
