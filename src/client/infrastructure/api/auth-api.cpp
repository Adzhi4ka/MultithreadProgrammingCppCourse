...
AuthApi::AuthApi(ApiClient& apiClient, QObject* parent) : QObject(parent), m_apiClient(apiClient) {}
...
void AuthApi::registerUser(const QString& login, const QString& password,
                           std::function<void(ApiResult<UserSession>)> callback) {
    authenticate("/api/auth/register", login, password, std::move(callback));
}

void AuthApi::authenticate(const QString& path, const QString& login, const QString& password,
                           std::function<void(ApiResult<UserSession>)> callback) {
    QJsonObject body{
        {"login", login},
        {"password", password},
