...
FileLockApi::FileLockApi(ApiClient& apiClient, QObject* parent) : QObject(parent), m_apiClient(apiClient) {}
...
void FileLockApi::renew(qint64 fileId, qint64 lockToken, qint64 lockDurationSec,
    ...
    m_apiClient.putJson("/api/file-locks/renew", body,
...
void FileLockApi::release(qint64 fileId, qint64 lockToken, std::function<void(ApiResult<void>)> callback) {
    ...
    m_apiClient.deleteRequest("/api/file-locks", query,
                              [callback = std::move(callback)](RawApiResponse response) mutable {
                                  if (!response.isSuccessStatus()) {
