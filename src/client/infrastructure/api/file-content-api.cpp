...
namespace client::infrastructure::api {
    ...
    FileContentApi::FileContentApi(ApiClient& apiClient, QObject* parent)
        : QObject(parent),
    ...
    void FileContentApi::uploadCurrent(qint64 fileId, const QByteArray& content, VersionCallback callback) {
        ...
        m_apiClient.putRaw("/api/files/content",
                           query,
                           content,
    ...
    void FileContentApi::downloadVersion(qint64 versionId, BytesCallback callback) {
        ...
        m_apiClient.get("/api/file-versions/content", query, [callback = std::move(callback)](RawApiResponse response) mutable {
            if (!response.isSuccessStatus()) {
