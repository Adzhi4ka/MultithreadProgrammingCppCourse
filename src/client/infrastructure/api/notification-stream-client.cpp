...
namespace client::infrastructure::api {
    ...
    void NotificationStreamClient::setBaseUrl(QUrl baseUrl) {
        m_baseUrl = std::move(baseUrl);
    }

    void NotificationStreamClient::setBearerToken(QString token) {
        m_bearerToken = std::move(token);
    }

    void NotificationStreamClient::clearBearerToken() {
        m_bearerToken.clear();
    }
    ...
    void NotificationStreamClient::dispatchCurrentEvent() {
        ...
        domain::models::NotificationEvent event{
            .name = m_currentEventName,
            .payload = json.object()
        };
