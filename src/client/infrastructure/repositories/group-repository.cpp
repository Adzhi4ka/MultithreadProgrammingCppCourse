...
void GroupRepository::upsertGroup(Group group) { m_groupsById.insert(group.id, std::move(group)); }

void GroupRepository::upsertUser(UserProfile user) { m_usersById.insert(user.userId, std::move(user)); }
...
void GroupRepository::replaceGroupUsers(qint64 groupId, std::vector<UserProfile> users) {
    ...
    std::sort(users.begin(), users.end(), [](const auto& lhs, const auto& rhs) { return lhs.userId < rhs.userId; });

    users.erase(std::unique(users.begin(), users.end(),
                            [](const auto& lhs, const auto& rhs) { return lhs.userId == rhs.userId; }),
                users.end());

    std::sort(users.begin(), users.end(),
              [](const auto& lhs, const auto& rhs) { return lhs.login.localeAwareCompare(rhs.login) < 0; });
