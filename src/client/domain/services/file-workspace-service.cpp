...
FileWorkspaceService::FileWorkspaceService(application::NetworkWorker& networkWorker, QObject& internalContext,
                                           QObject& uiContext,
...
void FileWorkspaceService::uploadCurrent(qint64 fileId, QByteArray content,
                                         std::function<void(ApiResult<FileVersion>)> callback) {
    runSimple(std::move(callback),
              [fileId, content = std::move(content)](RemoteApiGateway& gateway, auto done) mutable {
...
void FileWorkspaceService::acquireLock(qint64 fileId, qint64 lockDurationSec,
    ...
    m_networkWorker.run(
        [fileId, lockDurationSec, internalContext, uiContext, cb, fileRepository](RemoteApiGateway& gateway) mutable {
            gateway.lockApi().acquire(
                fileId, lockDurationSec,
                [fileId, internalContext, uiContext, cb, fileRepository](ApiResult<FileLock> result) mutable {
...
void FileWorkspaceService::renewLock(qint64 fileId, qint64 lockToken, qint64 lockDurationSec,
...
void FileWorkspaceService::releaseLock(qint64 fileId, qint64 lockToken, std::function<void(ApiResult<void>)> callback) {
    ...
    m_networkWorker.run(
        [fileId, lockToken, internalContext, uiContext, cb, fileRepository](RemoteApiGateway& gateway) mutable {
            gateway.lockApi().release(
                fileId, lockToken,
                [fileId, internalContext, uiContext, cb, fileRepository](ApiResult<void> result) mutable {
                    ::client::application::postTask(
                        internalContext, [fileId, uiContext, cb, fileRepository, result = std::move(result)]() mutable {
                            if (result) {
                                if (auto file = fileRepository->findById(fileId)) {
...
void FileWorkspaceService::loadVersions(qint64 fileId,
