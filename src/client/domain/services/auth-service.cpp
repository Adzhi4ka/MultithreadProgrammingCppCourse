...
AuthService::AuthService(application::NetworkWorker& networkWorker, QObject& internalContext, QObject& uiContext,
                         infrastructure::repositories::SessionRepository& sessionRepository) noexcept
    : RemoteServiceBase(networkWorker, internalContext, uiContext), m_sessionRepository(sessionRepository) {}

void AuthService::authenticate(QUrl baseUrl, QString login, QString password, bool registration,
    ...
    m_networkWorker.run([baseUrl = std::move(baseUrl), login = std::move(login), password = std::move(password),
                         registration, internalContext, uiContext, cb,
                         sessionRepository](RemoteApiGateway& gateway) mutable {
        gateway.setBaseUrl(std::move(baseUrl));
        gateway.clearBearerToken();

        auto* gatewayPtr = &gateway;
        auto done = [gatewayPtr, internalContext, uiContext, cb,
                     sessionRepository](ApiResult<UserSession> result) mutable {
            if (result) {
...
void AuthService::setSession(UserSession session) {
    ...
    m_networkWorker.run([session = std::move(session)](RemoteApiGateway& gateway) mutable {
...
void AuthService::logout() {
    ...
    m_networkWorker.run([](RemoteApiGateway& gateway) { gateway.clearBearerToken(); });
}
