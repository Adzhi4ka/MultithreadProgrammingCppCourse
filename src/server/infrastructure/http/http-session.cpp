...
HttpSession::HttpSession(tcp::socket&& socket, Router& router) : m_stream(std::move(socket)), m_router(router) {}

void HttpSession::run() {
    net::dispatch(m_stream.get_executor(), beast::bind_front_handler(&HttpSession::doRead, shared_from_this()));
}

net::any_io_executor HttpSession::executor() noexcept { return m_stream.get_executor(); }

void HttpSession::sendResponse(Response response) {
    net::post(m_stream.get_executor(), [self = shared_from_this(), response = std::move(response)]() mutable {
...
void HttpSession::onRead(beast::error_code ec, std::size_t) {
    ...
    if (!context.hasResponse()) {
        context.reply(makeTextResponse(http::status::internal_server_error, "Handler did not produce response",
                                       context.request().version(), context.request().keep_alive()));
    }
...
void HttpSession::writeResponse(Response response) {
    ...
    beast::async_write(m_stream, std::move(response),
