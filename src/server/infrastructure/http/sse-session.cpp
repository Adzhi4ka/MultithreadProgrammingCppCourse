...
SseSession::SseSession(beast::tcp_stream&& stream, int64_t userId, uint64_t sessionId,
...
void SseSession::start() {
    net::dispatch(m_strand, beast::bind_front_handler(&SseSession::doStart, shared_from_this()));
}
...
void SseSession::sendEvent(std::string eventName, std::string jsonData) {
    net::post(m_stream.get_executor(),
              [self = shared_from_this(), eventName = std::move(eventName), jsonData = std::move(jsonData)]() mutable {
                  if (self->m_closed) {
                      return;
                  }
...
void SseSession::sendComment(std::string comment) {
    net::post(m_stream.get_executor(), [self = shared_from_this(), comment = std::move(comment)]() mutable {
        if (self->m_closed) {
            return;
        }
...
void SseSession::close() {
    net::post(m_stream.get_executor(), [self = shared_from_this()] { self->doClose(); });
}

int64_t SseSession::userId() const noexcept { return m_userId; }

uint64_t SseSession::sessionId() const noexcept { return m_sessionId; }
...
void SseSession::doWrite() {
    ...
    net::async_write(m_stream, net::buffer(m_outbox.front()),
                     net::bind_executor(m_strand, beast::bind_front_handler(&SseSession::onWrite, shared_from_this())));
}
