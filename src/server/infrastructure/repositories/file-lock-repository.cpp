...
inline FileLock readFromStatement(const SQLite::Statement& stmt) {
    int readIndex = 0;
    return FileLock{.fileId = stmt.getColumn(readIndex++).getInt64(),
...
PersistenceResult<void> FileLockRepository::updateLease(WriteUnitOfWork& wuow, int64_t fileId, int64_t lockToken,
                                                        int64_t leaseUntil) {
    constexpr const char* const sql = {
        "UPDATE file_lock "
        "SET lease_until = ? "
