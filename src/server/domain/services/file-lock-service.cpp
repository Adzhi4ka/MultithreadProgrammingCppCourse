...
FileLockService::FileLockService(SqliteDatabase& database, FileLockRepository& fileLockRepo) noexcept
    : m_database(database), m_fileLockRepo(fileLockRepo) {}

ServiceResult<FileLock> FileLockService::acquireLock(int64_t fileId, int64_t userId, int64_t lockDurationSec) {
...
ServiceResult<void> FileLockService::renewLock(int64_t fileId, int64_t lockToken, int64_t lockDurationSec) {

    /*
        По факту тут есть окно, пока мы пытаемся потом захватить WriteUnitOfWork, в течение которого время может сильно
       измениться Но мне кажется, что это время столь мало, что не так страшно
    */
    {
    ...
    auto renewResult = m_fileLockRepo.updateLease(wuow, fileId, lockToken, unixNowSeconds() + lockDurationSec);
