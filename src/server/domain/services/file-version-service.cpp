...
FileVersionService::FileVersionService(SqliteDatabase& database, FileRepository& fileRepo,
                                       FileVersionRepository& fileVersionRepo) noexcept
    : m_database(database), m_fileRepo(fileRepo), m_fileVersionRepo(fileVersionRepo) {}

ServiceResult<int64_t> FileVersionService::createNewVersion(int64_t fileId, const std::string& logicalNameSnapshot,
