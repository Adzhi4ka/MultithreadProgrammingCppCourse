...
GroupService::GroupService(SqliteDatabase& database, GroupRepository& groupRepo,
                           UserGroupRepository& userGroupRepo) noexcept
    : m_database(database), m_groupRepo(groupRepo), m_userGroupRepo(userGroupRepo) {}
...
ServiceResult<int64_t> GroupService::createGroup(const std::string& groupName) {
    ...
    auto createResult = m_groupRepo.create(wuow, Group{.id = newId, .name = groupName});
...
ServiceResult<void> GroupService::addUserToGroup(int64_t userId, int64_t groupId) {
    ...
    auto addResult = m_userGroupRepo.addUserToGroup(wuow, UserGroup{.userId = userId, .groupId = groupId});
...
ServiceResult<void> GroupService::removeUserFromGroup(int64_t userId, int64_t groupId) {
    ...
    auto removeResult = m_userGroupRepo.removeUserFromGroup(wuow, UserGroup{.userId = userId, .groupId = groupId});
