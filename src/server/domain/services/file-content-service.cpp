...
ServiceResult<CreatedFileStorage> FileContentService::createNew() {
    ...
    try {
        ...
        return CreatedFileStorage{.physicalPath = physicalPath, .storage = std::move(newFileStorage)};
...
ServiceResult<DownloadFileStorage> FileContentService::openDownload(uint64_t physicalPath) {
    try {
        ...
        return DownloadFileStorage{.path = FileStorage::makePath(physicalPath), .size = (uint64_t)fileSize};
    } catch (const std::system_error& ex) {
        if (ex.code().value() == ENOENT) {
            return std::unexpected(ServiceError::NotFound);
        }
    ...
    try {
        ...
        while (totalWritten < content.size()) {
            const auto writtenBytes =
                ::write(storage.getFd(), content.data() + totalWritten, content.size() - totalWritten);
...
uint64_t FileContentService::generatePhysicalName() { return infrastructure::id_generator::generateId(); }
