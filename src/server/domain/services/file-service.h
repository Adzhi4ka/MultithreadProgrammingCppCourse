...
class FileService {
    ...
    public:
        ...
        FileService(SqliteDatabase& database, FileRepository& fileRepo, FileVersionRepository& fileVersionRepo,
                    UserRepository& userRepo, GroupRepository& groupRepo, FileAclRepository& fileAclRepo) noexcept;

        ServiceResult<int64_t> create(std::string logicalName, int64_t createdByUser, uint64_t physicalPath,
