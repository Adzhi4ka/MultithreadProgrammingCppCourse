...
FileAclService::FileAclService(SqliteDatabase& database, FileAclRepository& fileAclRepo,
                               UserGroupRepository& userGroupRepo) noexcept
    : m_database(database), m_fileAclRepo(fileAclRepo), m_userGroupRepo(userGroupRepo) {}
...
ServiceResult<void> FileAclService::setGroupAclLevel(int64_t fileId, int64_t groupId, AclLevel aclLevel) {
    auto wuow = m_database.createWriteUnitOfWork();

    auto setResult = m_fileAclRepo.grant(wuow, FileAcl{.fileId = fileId, .groupId = groupId, .aclLevel = aclLevel});
