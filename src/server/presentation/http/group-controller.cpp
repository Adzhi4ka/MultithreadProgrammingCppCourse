...
json::object toJson(const Group& group) { return json::object{{"id", group.id}, {"name", group.name}}; }
...
GroupController::GroupController(GroupService& groupService, AuthTokenStore& tokenStore,
                                 ThreadPool& threadPool) noexcept
    : m_groupService(groupService), m_tokenStore(tokenStore), m_threadPool(threadPool) {}

GroupController::GroupController(GroupService& groupService, AuthTokenStore& tokenStore, ThreadPool& threadPool,
...
void GroupController::registerRoutes(Router& router) {
    router.add(http::verb::post, "/api/groups", [this](RouteContext& ctx) { handleCreateGroup(ctx); });

    router.add(http::verb::delete_, "/api/groups", [this](RouteContext& ctx) { handleDeleteGroup(ctx); });

    router.add(http::verb::get, "/api/groups", [this](RouteContext& ctx) { handleGetAllGroups(ctx); });

    router.add(http::verb::get, "/api/groups/by-id", [this](RouteContext& ctx) { handleGetGroupById(ctx); });

    router.add(http::verb::post, "/api/groups/members", [this](RouteContext& ctx) { handleAddUserToGroup(ctx); });

    router.add(http::verb::delete_, "/api/groups/members",
               [this](RouteContext& ctx) { handleRemoveUserFromGroup(ctx); });

    router.add(http::verb::get, "/api/groups/by-user", [this](RouteContext& ctx) { handleGetUserGroups(ctx); });

    router.add(http::verb::get, "/api/groups/users", [this](RouteContext& ctx) { handleGetGroupUsers(ctx); });
}
...
void GroupController::handleCreateGroup(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, groupNameValue]() -> infrastructure::http::Response {
        ...
        return infrastructure::http::makeJsonResponse(
            http::status::created, serializeJson({{"id", *result}, {"name", groupNameValue}}), version, keepAlive);
    };
...
void GroupController::handleGetAllGroups(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive]() -> infrastructure::http::Response {
        ...
        return infrastructure::http::makeJsonResponse(
            http::status::ok, serializeJson(json::object{{"items", toJsonArray(*result)}}), version, keepAlive);
    };
...
void GroupController::handleAddUserToGroup(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, userId = *userId, groupId = *groupId,
...
void GroupController::handleGetUserGroups(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, userId = *userId]() -> infrastructure::http::Response {
        ...
        return infrastructure::http::makeJsonResponse(
            http::status::ok, serializeJson(json::object{{"items", toJsonArray(*result, "groupId")}}), version,
            keepAlive);
    };
...
void GroupController::handleGetGroupUsers(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, groupId = *groupId]() -> infrastructure::http::Response {
        ...
        return infrastructure::http::makeJsonResponse(
            http::status::ok, serializeJson(json::object{{"items", toJsonArray(*result, "userId")}}), version,
            keepAlive);
    };
