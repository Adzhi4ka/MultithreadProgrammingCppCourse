...
void AuthController::registerRoutes(infrastructure::http::Router& router) {
    router.add(http::verb::post, "/api/auth/register",
               [this](infrastructure::http::RouteContext& ctx) { handleRegister(ctx); });

    router.add(http::verb::post, "/api/auth/login",
               [this](infrastructure::http::RouteContext& ctx) { handleLogin(ctx); });
}
...
void AuthController::handleRegister(infrastructure::http::RouteContext& ctx) {
    ...
    if (!body) {
        ctx.reply(infrastructure::http::makeJsonResponse(http::status::bad_request, R"({"error":"invalid_json"})",
    ...
    dispatchToWorker(ctx, m_threadPool,
                     [this, version, keepAlive, loginValue, passwordValue]() -> infrastructure::http::Response {
                         ...
                         json::object responseBody{{"userId", *result}, {"login", loginValue}, {"token", token}};

                         return infrastructure::http::makeJsonResponse(http::status::created,
                                                                       serializeJson(responseBody), version, keepAlive);
                     });
}
    ...
    dispatchToWorker(ctx, m_threadPool,
                     [this, version, keepAlive, loginValue, passwordValue]() -> infrastructure::http::Response {
                         ...
                         json::object responseBody{
                             {"userId", *result}, {"login", loginValue}, {"token", m_tokenStore.issueToken(*result)}};

                         return infrastructure::http::makeJsonResponse(http::status::ok, serializeJson(responseBody),
                                                                       version, keepAlive);
                     });
}

infrastructure::http::Response AuthController::makeServiceErrorResponse(unsigned version, bool keepAlive,
    ...
    switch (error) {
        ...
        case domain::services::ServiceError::Forbidden:
            return infrastructure::http::makeJsonResponse(http::status::unauthorized,
                                                          R"({"error":"invalid_credentials"})", version, keepAlive);
