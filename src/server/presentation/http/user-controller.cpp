...
namespace {
    ...
    json::object toJson(const User& user) {
        return json::object{{"userId", user.id},
                            {"login", user.login}};
    }
...
namespace presentation::http {
    ...
    void UserController::registerRoutes(Router& router) {
        router.add(http::verb::get, "/api/users/by-id",
                   [this](RouteContext& ctx) { handleGetById(ctx); });

        router.add(http::verb::get, "/api/users/by-login",
                   [this](RouteContext& ctx) { handleGetByLogin(ctx); });
    }
