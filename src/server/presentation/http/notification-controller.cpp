...
NotificationController::NotificationController(infrastructure::http::ActiveSessionRegistry& registry,
    ...
    : m_registry(registry), m_tokenStore(tokenStore) {}
...
void NotificationController::registerRoutes(infrastructure::http::Router& router) {
    router.add(http::verb::get, "/api/notifications/stream",
               [this](infrastructure::http::RouteContext& ctx) { handleStream(ctx); });
}
...
void NotificationController::handleStream(infrastructure::http::RouteContext& ctx) {
    ...
    auto sseSession = std::make_shared<infrastructure::http::SseSession>(
        std::move(stream), *userId, sessionId,
        [this](int64_t closedUserId, uint64_t closedSessionId) { m_registry.remove(closedUserId, closedSessionId); });

    m_registry.add(sseSession);
    sseSession->start();

    json::object payload{{"userId", *userId}};
