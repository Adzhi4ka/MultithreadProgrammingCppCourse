...
inline std::optional<std::string> getStringField(const json::object& object, std::string_view fieldName) {
...
inline std::optional<int64_t> getInt64Field(const json::object& object, std::string_view fieldName) {
    const auto it = object.find(fieldName);
    if (it == object.end()) {
        return std::nullopt;
    }
...
inline std::optional<std::string_view> getQueryParam(std::string_view target, std::string_view key) {
...
inline std::string serializeJson(const json::value& value) { return json::serialize(value); }
