...
template <class Work>
void dispatchToWorker(infrastructure::http::RouteContext& ctx, infrastructure::execution::ThreadPool& threadPool,
                      Work&& work) {
    ...
    threadPool.post([responder = std::move(responder), work = std::forward<Work>(work), version, keepAlive]() mutable {
        try {
            responder.send(work());
        } catch (...) {
