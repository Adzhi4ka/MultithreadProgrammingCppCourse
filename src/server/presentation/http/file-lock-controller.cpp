...
json::object toJson(const FileLock& fileLock) {
    return json::object{{"fileId", fileLock.fileId},
...
FileLockController::FileLockController(FileLockService& fileLockService, AuthTokenStore& tokenStore,
                                       ThreadPool& threadPool) noexcept
    : m_fileLockService(fileLockService), m_tokenStore(tokenStore), m_threadPool(threadPool) {}

FileLockController::FileLockController(FileLockService& fileLockService, AuthTokenStore& tokenStore,
                                       ThreadPool& threadPool, NotificationPublisher& notificationPublisher) noexcept
...
void FileLockController::registerRoutes(Router& router) {
    router.add(http::verb::post, "/api/file-locks", [this](RouteContext& ctx) { handleAcquireLock(ctx); });

    router.add(http::verb::put, "/api/file-locks/renew", [this](RouteContext& ctx) { handleRenewLock(ctx); });

    router.add(http::verb::delete_, "/api/file-locks", [this](RouteContext& ctx) { handleReleaseLock(ctx); });

    router.add(http::verb::get, "/api/file-locks/active", [this](RouteContext& ctx) { handleGetActiveLock(ctx); });
}
...
void FileLockController::handleAcquireLock(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, fileId = *fileId, userId = *authenticatedUserId,
                 lockDurationSec]() -> infrastructure::http::Response {
        auto result = lockDurationSec ? m_fileLockService.acquireLock(fileId, userId, *lockDurationSec)
        ...
        if (m_notificationPublisher) {
            m_notificationPublisher->fileLocked(result->fileId, result->userId, result->leaseUntil, result->lockToken);
        }

        return infrastructure::http::makeJsonResponse(http::status::created, serializeJson(toJson(*result)), version,
                                                      keepAlive);
    };
...
void FileLockController::handleRenewLock(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, fileId = *fileId, lockToken = *lockToken,
                 lockDurationSec]() -> infrastructure::http::Response {
        auto result = lockDurationSec ? m_fileLockService.renewLock(fileId, lockToken, *lockDurationSec)
...
void FileLockController::handleReleaseLock(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, fileId = *fileId,
                 lockToken = *lockToken]() -> infrastructure::http::Response {
        ...
        if (m_notificationPublisher) {
            m_notificationPublisher->fileUnlocked(fileId, lockToken);
        }
