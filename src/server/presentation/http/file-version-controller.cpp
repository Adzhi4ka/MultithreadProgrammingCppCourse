...
void FileVersionController::registerRoutes(Router& router) {
    router.add(http::verb::post, "/api/file-versions", [this](RouteContext& ctx) { handleCreateNewVersion(ctx); });

    router.add(http::verb::get, "/api/file-versions/current",
               [this](RouteContext& ctx) { handleGetCurrentVersion(ctx); });

    router.add(http::verb::get, "/api/file-versions", [this](RouteContext& ctx) { handleGetAllVersions(ctx); });
}
...
void FileVersionController::handleCreateNewVersion(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, fileId = *fileId,
        ...
        auto createVersionResult = m_fileVersionService.createNewVersion(fileId, logicalNameSnapshot, physicalPath);
