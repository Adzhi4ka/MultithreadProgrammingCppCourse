...
namespace {
    ...
    inline std::optional<int64_t> getRequiredInt64QueryParam(
        const infrastructure::http::Request& req,
    ...
    inline bool requireFileAcl(infrastructure::http::RouteContext& ctx,
        ...
        if (*aclResult < requiredAclLevel) {
            ctx.reply(presentation::http::makeServiceErrorResponse(req,
                                                                   domain::services::ServiceError::Forbidden));
            return false;
        }
    ...
    infrastructure::http::Response makeFileBodyResponse(unsigned version,
                                                        bool keepAlive,
        ...
        if (ec) {
            return presentation::http::makeServiceErrorResponse(version,
                                                               keepAlive,
...
namespace presentation::http {
    ...
    void FileContentController::registerRoutes(Router& router) {
        router.add(http::verb::get, "/api/files/content",
                   [this](RouteContext& ctx) {
                       handleDownloadCurrentContent(ctx);
                   });

        router.add(http::verb::put, "/api/files/content",
                   [this](RouteContext& ctx) {
                       handleUploadCurrentContent(ctx);
                   });

        router.add(http::verb::get, "/api/file-versions/content",
                   [this](RouteContext& ctx) {
                       handleDownloadVersionContent(ctx);
                   });
    }
    ...
    void FileContentController::handleUploadCurrentContent(RouteContext& ctx) {
        ...
        auto task = [this,
            ...
            auto createVersionResult = m_fileVersionService.createNewVersion(fileId,
                                                                             previousVersionResult->logicalNameSnapshot,
                                                                             physicalPath);
    ...
    void FileContentController::handleDownloadVersionContent(RouteContext& ctx) {
        ...
        auto task = [this,
            ...
            if (*aclResult < AclLevel::READ_ONLY) {
                return makeServiceErrorResponse(version,
                                                keepAlive,
                                                domain::services::ServiceError::Forbidden);
            }
