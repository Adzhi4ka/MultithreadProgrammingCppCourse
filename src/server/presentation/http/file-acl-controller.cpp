...
std::optional<AclLevel> parseAclLevelField(const json::object& object, std::string_view fieldName) {
...
json::object toJson(const FileAcl& fileAcl) {
    return json::object{{"fileId", fileAcl.fileId},
...
FileAclController::FileAclController(FileAclService& fileAclService, AuthTokenStore& tokenStore,
                                     ThreadPool& threadPool) noexcept
    : m_fileAclService(fileAclService), m_tokenStore(tokenStore), m_threadPool(threadPool) {}

void FileAclController::registerRoutes(infrastructure::http::Router& router) {
    router.add(http::verb::put, "/api/file-acl/groups", [this](RouteContext& ctx) { handleSetGroupAcl(ctx); });

    router.add(http::verb::delete_, "/api/file-acl/groups", [this](RouteContext& ctx) { handleRemoveGroupAcl(ctx); });

    router.add(http::verb::get, "/api/file-acl/groups", [this](RouteContext& ctx) { handleGetGroupAcl(ctx); });

    router.add(http::verb::get, "/api/file-acl/users", [this](RouteContext& ctx) { handleGetUserAcl(ctx); });

    router.add(http::verb::get, "/api/file-acl/by-file", [this](RouteContext& ctx) { handleGetFileAcls(ctx); });

    router.add(http::verb::get, "/api/file-acl/by-group", [this](RouteContext& ctx) { handleGetGroupAcls(ctx); });
}
...
void FileAclController::handleSetGroupAcl(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, fileId = *fileId, groupId = *groupId,
                 aclLevel = *aclLevel]() -> infrastructure::http::Response {
        ...
        return infrastructure::http::makeJsonResponse(
            http::status::ok,
            serializeJson({{"fileId", fileId}, {"groupId", groupId}, {"aclLevel", std::string(toString(aclLevel))}}),
            version, keepAlive);
    };
...
void FileAclController::handleGetGroupAcl(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, fileId = *fileId, groupId = *groupId]() -> infrastructure::http::Response {
        ...
        return infrastructure::http::makeJsonResponse(
            http::status::ok,
            serializeJson({{"fileId", fileId}, {"groupId", groupId}, {"aclLevel", std::string(toString(*result))}}),
            version, keepAlive);
    };
...
void FileAclController::handleGetUserAcl(RouteContext& ctx) {
    ...
    auto task = [this, version, keepAlive, fileId = *fileId, userId = *userId]() -> infrastructure::http::Response {
        ...
        return infrastructure::http::makeJsonResponse(
            http::status::ok,
            serializeJson({{"fileId", fileId}, {"userId", userId}, {"aclLevel", std::string(toString(*result))}}),
            version, keepAlive);
    };
